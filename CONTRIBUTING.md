...
6. Bump the illustrative `context-schema` value in `skills/keep-the-why/examples/first-time-setup.md`'s config block snippet to match — it's a hardcoded example version, not a placeholder, so it goes stale on every release that doesn't touch the `context/` format, same as this repo's own `AGENTS.md` would without step 5.
7. Tag `vX.Y.Z` and push the tag — the `GH Release` workflow creates the release and moves the `latest` tag automatically
