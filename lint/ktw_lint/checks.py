...
CONFIG_REQUIRED = ("context", "init", "context-schema", "capture-confirmation", "source-reference")
...
DEFAULTS_KNOWN = ("capture-mode", "confirmation-flow", "update-check", "consistency-check")
...
_SUSPICIOUS_CHARS = {chr(cp): unicodedata.name(chr(cp), f"U+{cp:04X}") for cp in _SUSPICIOUS_CODEPOINTS}
...
class Linter:
    ...
    def check_config(self, parsed: ParsedConfig):
        ...
        for key, occurrences in block.fields.items():
            if len(occurrences) > 1:
                ...
                self.add(ERROR, "E004", path, occurrences[1][0],
            ...
            if key not in CONFIG_KNOWN:
                self.add(ERROR, "E005", path, occurrences[0][0],
        ...
        for key in CONFIG_REQUIRED:
            if key not in block.fields:
                if key == "context-schema":
                    self.add(WARNING, "W001", path, block.start_line,
                             "config has no context-schema — assuming 0.2.0, the skill's own backfill default")
                else:
                    self.add(ERROR, "E002", path, block.start_line,
        ...
        if init_field and init_field[1] not in INIT_VALUES:
            self.add(ERROR, "E003", path, init_field[0],
        ...
        if cc_field and cc_field[1] not in CAPTURE_CONFIRMATION_VALUES:
            self.add(ERROR, "E003", path, cc_field[0],
        ...
        if sr_field:
            ...
            if value in ("always", "never"):
                pass
            elif head == "filtered" or value.startswith("filtered:") or value.startswith("filtered "):
                criteria = rest or value.partition("filtered")[2].lstrip(":—– -").strip()
                if not criteria:
                    self.add(ERROR, "E003", path, line,
        ...
        if not parsed.legacy and self.schema >= GATE_DEDICATED_CONFIG:
            ...
            if id_field is None:
                ...
                self.add(ERROR, "E003", path, id_field[0],
        ...
        if ctx_field:
            ...
            if not self._exists(self.context_dir):
                self.add(ERROR, "E007", path, ctx_field[0],
    ...
    def _check_personal_defaults(self, parsed: ParsedConfig):
        ...
        for key in ("update-check", "consistency-check"):
            ...
            if fld and not _INTERVAL_RE.match(fld[1].strip()):
                self.add(WARNING, "W002", path, fld[0],
    ...
    def _check_entry(self, path, entry):
        if not entry.fields:
            self.add(WARNING, "W102", path, entry.line,
        ...
        if self.schema >= GATE_STATUS_EVIDENCE:
            self._check_single_valued(path, entry, "Status", STATUS_VALUES, "E101", "E103")
            self._check_single_valued(path, entry, "Evidence", EVIDENCE_VALUES, "E102", "E104")
            for fld in entry.get("Verification"):
                match = _VERIFICATION_RE.match(fld.value.strip())
                if not match:
                    self.add(ERROR, "E106", path, fld.line,
                    ...
                    self.add(ERROR, "E111", path, fld.line,
        ...
        for fld in entry.get("Revisit when"):
            if not fld.value.strip():
                self.add(WARNING, "W105", path, fld.line, "empty 'Revisit when' condition")

    def _check_single_valued(self, path, entry, name, allowed, missing_code, invalid_code):
        fields = entry.get(name)
        if not fields:
            self.add(ERROR, missing_code, path, entry.line,
                     f"'{entry.title}': entry has no **{name}:** field")
            return
        if len(fields) > 1:
            self.add(ERROR, "E112", path, fields[1].line,
                     f"'{entry.title}': more than one **{name}:** line")
        value = fields[0].value.strip()
        if value not in allowed:
            self.add(ERROR, invalid_code, path, fields[0].line,
    ...
    def _check_type(self, path, entry):
        ...
        if self.schema < GATE_TYPE:
            if type_fields:
                self.add(WARNING, "W104", path, type_fields[0].line,
        ...
        if not type_fields:
            self.add(WARNING, "W101", path, entry.line,
        ...
        for fld in type_fields:
            ...
            if head == "undefined":
                if self.schema < GATE_UNDEFINED:
                    self.add(ERROR, "E105", path, fld.line,
                             "Type 'undefined' needs context-schema >= 0.8.0")
                elif not rest:
                    self.add(ERROR, "E107", path, fld.line,
        ...
        if len(type_fields) > 1:
            if self.schema < GATE_MULTI_TYPE:
                self.add(ERROR, "E110", path, type_fields[1].line,
                         "multiple **Type:** lines need context-schema >= 0.9.0")
            if "undefined" in heads:
                self.add(ERROR, "E108", path, type_fields[0].line,
                         "'undefined' never combines with other Type values — it means none of them fit")
            seen = set()
            for fld, head in zip(type_fields, heads):
                if head in seen:
                    self.add(ERROR, "E109", path, fld.line, f"duplicate Type value '{head}'")
                seen.add(head)

        # Field order: Type is documented as placed before Status.
        status_fields = entry.get("Status")
        if status_fields and type_fields and type_fields[0].line > status_fields[0].line:
            self.add(WARNING, "W103", path, type_fields[0].line,
    ...
    def check_index(self, topic_files):
        ...
        if not self._exists(index_path):
            self.add(ERROR, "E201", index_path, 0,
        ...
        for line, _text, target in rows:
            ...
            if target not in topic_files:
                self.add(ERROR, "E202", index_path, line,
        ...
        for topic in topic_files:
            if topic not in listed_names:
                self.add(ERROR, "E203", index_path, 0,
                         f"topic file '{topic}' is not listed in index.md")
        if self.schema >= GATE_DEDICATED_CONFIG and listed_names != sorted(listed_names):
            self.add(ERROR, "E204", index_path, listed[0][0] if listed else 0,
    ...
    def check_guard_files(self):
        ...
        for name in ("README.md", "AGENTS.md", "CLAUDE.md"):
            if not self._exists(os.path.join(self.context_dir, name)):
                self.add(WARNING, "W201", os.path.join(self.context_dir, name), 0,
    ...
    def check_hidden_content(self, relpath: str):
        ...
        for lineno, line in enumerate(text.splitlines(), start=1):
            ...
            for char, name in _SUSPICIOUS_CHARS.items():
                if char in line:
                    self.add(ERROR, "E301", relpath, lineno,
            ...
            for match in _BASE64_RE.finditer(line):
                ...
                self.add(WARNING, "W301", relpath, lineno,
    ...
    def run(self, parsed: ParsedConfig | None):
        if parsed is None:
            self.add(ERROR, "E001", ".keep-the-why", 0,
        ...
        all_md = sorted(
            name for name in os.listdir(context_abs)
