...
3. **Adapt to what exists.** Preserve the project's terminology and conventions. Update existing topic files instead of creating near-duplicates. Organize by *topic* (`auth.md`, `sync.md`), not by source file or commit.
...
Check for two independent config files: a project one (`.keep-the-why`, at the project root) and a personal one (`~/.keep-the-why/<id>.md`). See `references/setup.md` for format, detection logic, and exactly how `<id>` is derived.

**Project file missing:**
- Check for a legacy config block in `AGENTS.md` → if found, this is a migration: see `references/migrations.md`.
...
**Personal file missing → MUST run the personal preferences wizard now, in this turn.** Check for a legacy personal block in `AGENTS.local.md` first (see `references/migrations.md`). If genuinely absent, ask at least the first wizard question before doing anything else. See `references/setup.md` for the full wizard.
...
Three checks before writing: is this worth documenting at this depth (rule 10)? Which file does it belong in — `context/` isn't the only place; see "Which file does this belong in?" in `references/repository-structure.md`? Does it pass the privacy filter (rule 7)?
...
Before the actual write, resolve the effective `capture-confirmation` setting (rule 8).
